...
from leaderboard_pdf import generate_leaderboard_pdf
...
async def leaderboard_cmd(
    ...
    pdf_path = None
    try:
        pdf_path = generate_leaderboard_pdf(q, rows)
        ...
        with open(pdf_path, "rb") as document:
            await update.message.reply_document(
                document=document,
                caption=caption,
                filename=f"Leaderboard_{qid}.pdf",
            )

    except Exception as e:
        log.exception("Could not generate leaderboard PDF")
        await update.message.reply_text(
            f"❌ Leaderboard PDF नहीं बन सकी: {e}"
        )
    finally:
        if pdf_path:
            try:
...
async def pdf_cmd(
    ...
    file_path = None
    try:
        file_path = generate_quiz_pdf(q)
        filename = (
            f"Quick_Study_Group_{q['id']}.pdf"
        )
        ...
        await update.message.reply_text(
            "❌ PDF generate नहीं हो पाई। Render logs check करें।"
        )
    finally:
        if file_path:
            try:
